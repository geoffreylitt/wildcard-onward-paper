...
We developed two \emph{cell editors}: custom UI widgets for editing
values in the table.

One benefit that cell editors provide is enabling users to incorporate
their private information into a web UI. We created a datepicker widget
(based on the \href{https://fullcalendar.io/}{FullCalendar} plugin),
which can load calendar data from a Google Calendar. This makes it
convenient to enter dates into a website based on the user's personal
calendar information, without needing to upload a user's calendar to the
website itself.

Another benefit is that a user can choose a single preferred widget for
editing a certain type of information across different sites. For
...
demonstrate this capability, we built a text editor based on the
\href{https://ckeditor.com/}{CKEditor} rich text editor. We used the
editor with Google's Blogger website, by building a site adapter that
represented the contents of a blog post as a single table cell
