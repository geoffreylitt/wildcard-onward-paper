...
%DIF DEL initial-submission.tex   Sun Aug  9 12:56:33 2020
%DIF ADD paper.tex                Sat Aug 15 17:38:06 2020
...
\newenvironment{Shaded}{}{} %DIF > 
\newcommand{\AlertTok}[1]{\textcolor[rgb]{1.00,0.00,0.00}{\textbf{#1}}} %DIF > 
\newcommand{\AnnotationTok}[1]{\textcolor[rgb]{0.38,0.63,0.69}{\textbf{\textit{#1}}}} %DIF > 
\newcommand{\AttributeTok}[1]{\textcolor[rgb]{0.49,0.56,0.16}{#1}} %DIF > 
\newcommand{\BaseNTok}[1]{\textcolor[rgb]{0.25,0.63,0.44}{#1}} %DIF > 
\newcommand{\BuiltInTok}[1]{#1} %DIF > 
\newcommand{\CharTok}[1]{\textcolor[rgb]{0.25,0.44,0.63}{#1}} %DIF > 
\newcommand{\CommentTok}[1]{\textcolor[rgb]{0.38,0.63,0.69}{\textit{#1}}} %DIF > 
\newcommand{\CommentVarTok}[1]{\textcolor[rgb]{0.38,0.63,0.69}{\textbf{\textit{#1}}}} %DIF > 
\newcommand{\ConstantTok}[1]{\textcolor[rgb]{0.53,0.00,0.00}{#1}} %DIF > 
\newcommand{\ControlFlowTok}[1]{\textcolor[rgb]{0.00,0.44,0.13}{\textbf{#1}}} %DIF > 
\newcommand{\DataTypeTok}[1]{\textcolor[rgb]{0.56,0.13,0.00}{#1}} %DIF > 
\newcommand{\DecValTok}[1]{\textcolor[rgb]{0.25,0.63,0.44}{#1}} %DIF > 
\newcommand{\DocumentationTok}[1]{\textcolor[rgb]{0.73,0.13,0.13}{\textit{#1}}} %DIF > 
\newcommand{\ErrorTok}[1]{\textcolor[rgb]{1.00,0.00,0.00}{\textbf{#1}}} %DIF > 
\newcommand{\ExtensionTok}[1]{#1} %DIF > 
\newcommand{\FloatTok}[1]{\textcolor[rgb]{0.25,0.63,0.44}{#1}} %DIF > 
\newcommand{\FunctionTok}[1]{\textcolor[rgb]{0.02,0.16,0.49}{#1}} %DIF > 
\newcommand{\ImportTok}[1]{#1} %DIF > 
\newcommand{\InformationTok}[1]{\textcolor[rgb]{0.38,0.63,0.69}{\textbf{\textit{#1}}}} %DIF > 
\newcommand{\KeywordTok}[1]{\textcolor[rgb]{0.00,0.44,0.13}{\textbf{#1}}} %DIF > 
\newcommand{\NormalTok}[1]{#1} %DIF > 
\newcommand{\OperatorTok}[1]{\textcolor[rgb]{0.40,0.40,0.40}{#1}} %DIF > 
\newcommand{\OtherTok}[1]{\textcolor[rgb]{0.00,0.44,0.13}{#1}} %DIF > 
\newcommand{\PreprocessorTok}[1]{\textcolor[rgb]{0.74,0.48,0.00}{#1}} %DIF > 
\newcommand{\RegionMarkerTok}[1]{#1} %DIF > 
\newcommand{\SpecialCharTok}[1]{\textcolor[rgb]{0.25,0.44,0.63}{#1}} %DIF > 
\newcommand{\SpecialStringTok}[1]{\textcolor[rgb]{0.73,0.40,0.53}{#1}} %DIF > 
\newcommand{\StringTok}[1]{\textcolor[rgb]{0.25,0.44,0.63}{#1}} %DIF > 
\newcommand{\VariableTok}[1]{\textcolor[rgb]{0.10,0.09,0.49}{#1}} %DIF > 
\newcommand{\VerbatimStringTok}[1]{\textcolor[rgb]{0.25,0.44,0.63}{#1}} %DIF > 
\newcommand{\WarningTok}[1]{\textcolor[rgb]{0.38,0.63,0.69}{\textbf{\textit{#1}}}} %DIF > 
...
%% article.
\begin{abstract}
  \DIFdelbegin \DIFdel{In this paper we introduce }\DIFdelend \DIFaddbegin \DIFadd{Customizing software should be as easy as using it. But most
  ...
  \DIFadd{We introduce }\DIFaddend \emph{\DIFdelbegin \DIFdel{table-driven }\DIFdelend \DIFaddbegin \DIFadd{data-driven }\DIFaddend customization}, a new way for end users
  ...
  implements \DIFdelbegin \DIFdel{table-driven customization for existing applications }\DIFdelend \DIFaddbegin \DIFadd{data-driven customization on the web }\DIFaddend using web scraping.
  Through concrete examples, we show that this paradigm can support
  \DIFaddbegin \DIFadd{useful }\DIFaddend extensions to many real websites, \DIFdelbegin \DIFdel{ranging from sorting and filtering existing data to adding entire new features. We }\DIFdelend \DIFaddbegin \DIFadd{and we }\DIFaddend share reflections from
  our experiences using \DIFdelbegin \DIFdel{Wildcard, on both its strengths
  and limitations relative to other customization approaches.
}\DIFdelend \DIFaddbegin \DIFadd{the tool.
}

  \DIFaddend Finally, we \DIFdelbegin \DIFdel{explore how this paradigm might lead to new software architectures
  that encourage this form of end-user customization}\DIFdelend \DIFaddbegin \DIFadd{share our broader vision for data-driven customization: a
  ...
  the context of everyday software usage}\DIFaddend .
\end{abstract}
...
\begin{figure*}
\DIFdelbeginFL %DIFDELCMD < \hypertarget{fig:overview}{%
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=\textwidth]{media/overview.eps}
%DIFDELCMD < \caption{An overview of table-driven customization}\label{fig:overview}
%DIFDELCMD < }
%DIFDELCMD < %%%
...
\DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{To explore this idea in a real context, we }\DIFaddend have developed a browser
extension called Wildcard \DIFdelbegin \DIFdel{which }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend uses web scraping techniques to implement
\DIFdelbegin \DIFdel{table-driven }\DIFdelend \DIFaddbegin \DIFadd{data-driven }\DIFaddend customization for existing Web applications. \DIFdelbegin \DIFdel{In Section~\ref{sec:example}, we illustrate the end
user experience of table-driven customization through an example scenario, using Wildcard to add features to Hacker News .
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In Section~\ref{sec:architecture}, we explain the system architecture of
table-driven customization. We focus on the }\emph{\DIFdel{table adapter}}
%DIFAUXCMD
\DIFdel{abstraction, which allows many different types of underlying data to be
bidirectionally mapped to a table. We describe several types of table
adapters we
've built in Wildcard , and also describe future adapters
supported by
the general paradigm.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We have successfully used Wildcard to build customizations for }\DIFdelend \DIFaddbegin \DIFadd{We introduce
...
sharing reflections from customizing }\DIFaddend 11 different websites \DIFdelbegin \DIFdel{that serve }\DIFdelend \DIFaddbegin \DIFadd{in ways that
met }\DIFaddend our own personal needs.
\DIFaddbegin 

\DIFadd{Wildcard is just an initial proof of concept of data-driven
customization. }\DIFaddend In Section~\DIFdelbegin \DIFdel{\ref{sec:reflections}, we present reflections from this process.
We outline the kinds of customizations we were able to build, limitations we encountered, and some of the challenges of writing
scraping logic. }\DIFdelend \DIFaddbegin \DIFadd{\ref{sec:visions}, we discuss our broader
...
\DIFdelbegin \DIFdel{In Section~\ref{sec:themes}, we discuss some key themes from our work:
}%DIFDELCMD < 
...
\tightlist
\item
  \DIFaddbegin \emph{\DIFadd{Decoupling data from applications}}\DIFadd{: On the modern Web, data is
  ...
  \DIFaddend \emph{Customization by direct manipulation}: We discuss \DIFdelbegin \DIFdel{further how
  table-driven customization relates to }\DIFdelend \DIFaddbegin \DIFadd{how
  data-driven customization promotes }\DIFaddend a gentle slope of customization. We
  \DIFdelbegin \DIFdel{suggest }\DIFdelend \DIFaddbegin \DIFadd{propose }\DIFaddend that an important point on this slope is the ability to
...
\DIFdelbegin \DIFdel{Table-driven customization relates to existing workin many areas}\DIFdelend \DIFaddbegin \DIFadd{In Section~\ref{sec:related-work} we discuss connections to related
work}\DIFaddend . Our goals overlap with \DIFdelbegin \DIFdel{many }\DIFdelend software customization tools, and our
...
To \DIFdelbegin \DIFdel{illustrate the end }\DIFdelend \DIFaddbegin \DIFadd{concretely illustrate the }\DIFaddend user experience of \DIFdelbegin \DIFdel{table-driven }\DIFdelend \DIFaddbegin \DIFadd{data-driven
}\DIFaddend customization, we \DIFdelbegin \DIFdel{consider an example }\DIFdelend \DIFaddbegin \DIFadd{present a }\DIFaddend scenario of customizing
...
This scenario has shown a few examples of how \DIFdelbegin \DIFdel{table-driven
}\DIFdelend \DIFaddbegin \DIFadd{data-driven }\DIFaddend customizations
can help a user improve their experience of a website.
Section~\ref{sec:reflections} explains many other use cases \DIFaddbegin \DIFadd{and contexts
where the technique applies}\DIFaddend , but first we explain how the system works
internally.
...
\DIFdelbegin \DIFdel{Here we describe }\DIFdelend \DIFaddbegin \hypertarget{abstract-interface}{%
\subsubsection{Abstract interface}\label{abstract-interface}}

\DIFadd{We begin by describing }\DIFaddend the abstract interface fulfilled by a table
adapter\DIFdelbegin \DIFdel{,
and then describe specific types of table adapters we've built so far in
the Wildcard system}\DIFdelend .

\DIFdelbegin %DIFDELCMD < \hypertarget{abstract-interface}{%
%DIFDELCMD < \subsubsection{Abstract interface}\label{abstract-interface}}
%DIFDELCMD < 
...
\DIFaddbegin \DIFadd{Next, we present the three types of table adapters we have built in
...
\DIFaddend \hypertarget{dom-scraping-adapters}{%
\subsubsection{DOM scraping adapters}\label{dom-scraping-adapters}}

DOM scraping adapters \DIFdelbegin \DIFdel{are the essential component that enables }\DIFdelend \DIFaddbegin \DIFadd{enable }\DIFaddend Wildcard to interface with an existing
website UI. \DIFdelbegin \DIFdel{In addition to }\DIFdelend \DIFaddbegin \DIFadd{A DOM scraping adapter fulfills }\DIFaddend the standard web scraping
\DIFdelbegin \DIFdel{problem }\DIFdelend \DIFaddbegin \DIFadd{task }\DIFaddend of extracting a table of data from the DOM, \DIFdelbegin \DIFdel{a
scraping adapter must also manipulate the
DOM to reorder rows, edit form
entries, and inject annotations as the tableis edited}\DIFdelend \DIFaddbegin \DIFadd{but it also acts in the
reverse direction: manipulating the DOM to reflect edits to the table}\DIFaddend .

\DIFdelbegin \DIFdel{Because every website has unique content, we rely on programmers to
create a DOM scraping adapter for each
individual website to make it
available for customization in Wildcard. To make this approach viable, we have built a generic DOM scraping adapter which a programmer can
configure with the minimal site-specific parts}\DIFdelend \DIFaddbegin \DIFadd{In Wildcard, DOM scraping adapters are programmed manually for each
...
any end user to perform customizations on that website}\DIFaddend .

\DIFdelbegin \emph{\DIFdel{Basic metadata}}%DIFAUXCMD
\DIFdel{: The programmer specifies URL patterns that the adapter should activate on , and a set of column names and types to
return in the table}\DIFdelend \DIFaddbegin \DIFadd{In the future, other strategies for producing DOM scraping adapters
...
critical mass of adapters could serve the needs of many users}\DIFaddend .

\DIFdelbegin \emph{\DIFdel{Specifying scraping logic}}%DIFAUXCMD
\DIFdel{: The main component of a site-specific
...
data,the programmer must return a
unique identifier, and values for
each column in the table}\DIFdelend \DIFaddbegin \DIFadd{To make it easier to create these adapters, Wildcard provides a
...
edits back into the page}\DIFaddend . \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The generic adapter then wraps thisscraping function to implement the table adapter interface. }\DIFdelend For example, when the user sorts the table\DIFdelbegin \DIFdel{view,
the generic adapter synchronizes the sort order to the DOM
by
removing the DOM elements corresponding to }\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend the \DIFdelbegin \DIFdel{rows in the table and
reinserting them back into the page in the
  new order.
}\DIFdelend \DIFaddbegin \DIFadd{DOM elements representing the table rows are moved around in the DOM
to reflect the new sorted order.
}\DIFaddend 

\DIFdelbegin \emph{\DIFdel{Choosing an ID}}%DIFAUXCMD
\DIFdel{:
}\DIFdelend \DIFaddbegin \begin{figure*}

\begin{Shaded}
\begin{Highlighting}[]
\DIFaddFL{\KeywordTok{const}\NormalTok{ HNAdapter }\OperatorTok{=} \AttributeTok{createDomScrapingAdapter}\NormalTok{(}\OperatorTok{\{}
  \DataTypeTok{name}\OperatorTok{:} \StringTok{"Hacker News"}\OperatorTok{,}
}

  \DIFaddFL{\CommentTok{// Specify when the adapter should be enabled, based on current URL}
...
\NormalTok{    )}\OperatorTok{;}
  ...
  \DIFaddFL{\CommentTok{// Define the name and type of each column in the table}
...
\NormalTok{  ]}\OperatorTok{,}
  ...
  \DIFaddFL{\CommentTok{// Iterate over DOM elements, returning information about each row}
  \AttributeTok{scrapePage}\NormalTok{() }\OperatorTok{\{}
    \ControlFlowTok{return} \VariableTok{Array}\NormalTok{.}\AttributeTok{from}\NormalTok{(}\VariableTok{document}\NormalTok{.}\AttributeTok{querySelectorAll}\NormalTok{(}\StringTok{"tr.athing"}\NormalTok{)).}\AttributeTok{map}\NormalTok{((el) }\KeywordTok{=>} \OperatorTok{\{}
      ...
      \DIFaddFL{\ControlFlowTok{return} \OperatorTok{\{}
        ...
        \DIFaddFL{\CommentTok{// Return DOM elements corresponding to this row}
        ...
        \DIFaddFL{\CommentTok{// Return data for each column}
        ...
        \DIFaddFL{\CommentTok{// Specify where annotations should be injected, and what they should look like}
...
\caption{\DIFaddFL{Source code for the Hacker News scraper. Some details removed for brevity.}}\label{fig:scraper-code}
\end{figure*}

\DIFadd{Figure~\ref{fig:scraper-code} shows an example of the scraper code used
...
\tightlist
\item
  \texttt{\DIFadd{enabled}}\DIFadd{: defines when this adapter should run, usually based
  on the active URL in the browser.}\footnote{\DIFadd{Currently Wildcard can only
...
\item
  \texttt{\DIFadd{attributes}}\DIFadd{: defines a schema for the table, with a name and
  type for each column
}\item
  \texttt{\DIFadd{scrapePage}}\DIFadd{: defines a scraping function which returns an
  array of objects, each containing the data for a single row of the
  table.
}\end{itemize}

\DIFadd{Here are some of the concerns that emerge when building adapters in
practice:
}

\emph{\DIFadd{Choosing a row ID}}\DIFadd{: }\DIFaddend When possible, it is best to choose a
...
the type of the column. \DIFdelbegin \DIFdel{For example, if the scraping function returns a
}\texttt{\DIFdel{\textless{}div\textgreater{}}} %DIFAUXCMD
\DIFdel{containing the string ``1.23'',
and the column has a numeric type, the generic adapter extracts the
number 1.23. }\DIFdelend The advantage of returning a DOM element is that
...
\emph{\DIFdelbegin \DIFdel{Other site-specific customization}\DIFdelend \DIFaddbegin \DIFadd{Optional overrides}\DIFaddend }: \DIFdelbegin \DIFdel{The programmer can optionally
override other behavior of the generic DOM scraping
adapter, including
logic for when to re-run }\DIFdelend \DIFaddbegin \DIFadd{In order to turn a unidirectional scraping
...
\tightlist
\item
  \DIFadd{when should }\DIFaddend the scraping function \DIFaddbegin \DIFadd{be re-run }\DIFaddend in response to \DIFdelbegin \DIFdel{page
changes , how to style injected annotations , and how to style the currently selected }\DIFdelend \DIFaddbegin \DIFadd{changes on
  the page?
}\item
  \DIFadd{how should injected annotations appear in table rows?
}\item
  \DIFadd{when the user selects a row in the table, how should the corresponding
  }\DIFaddend row in the \DIFdelbegin \DIFdel{table. We've chosen }\DIFdelend \DIFaddbegin \DIFadd{DOM be highlighted?
}\end{itemize}

\DIFadd{The scraping framework defines }\DIFaddend sensible defaults that \DIFdelbegin \DIFdel{generally result in a workable adapter without needing to change these
options, but they can improve the experience on a specific website}\DIFdelend \DIFaddbegin \DIFadd{work well on many
sites, but the programmer can optionally override them to provide better
site-specific behavior}\DIFaddend . For example, \DIFdelbegin \DIFdel{by default we highlight the currently selected row with a blue
border, but on some sites this is hard to see or breaks the layout, so
setting a background color works better}\DIFdelend \DIFaddbegin \DIFadd{the Hacker News adapter specifies
annotation options which make user annotations appear more naturally in
the context of the original design}\DIFaddend .
...
\DIFdelbegin %DIFDELCMD < \hypertarget{future-adapters}{%
%DIFDELCMD < \subsubsection{Future Adapters}\label{future-adapters}}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\textbf{\DIFdel{Integrated website adapters}}%DIFAUXCMD
...
}\href{https://redux.js.org/}{\DIFdel{Redux}} %DIFAUXCMD
...
%DIFDELCMD < %%%
\textbf{\DIFdel{Shared storage adapter}}%DIFAUXCMD
...
\end{tabular}
\caption{A list of \DIFdelbeginFL \DIFdelFL{table-driven }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{data-driven }\DIFaddendFL customizations that we have implemented using Wildcard.}
\label{tab:websites}
\end{table*}

To evaluate \DIFdelbegin \DIFdel{table-driven }\DIFdelend \DIFaddbegin \DIFadd{data-driven }\DIFaddend customization in practice, we built the Wildcard
browser extension, which implements \DIFdelbegin \DIFdel{table-driven }\DIFdelend \DIFaddbegin \DIFadd{data-driven }\DIFaddend customization in the
...
We developed two \DIFdelbegin \DIFdel{cell editors, to explore the benefits of adding custom UI }\DIFdelend \DIFaddbegin \emph{\DIFadd{cell editors}}\DIFadd{: custom UI widgets }\DIFaddend for editing
values in the table.

One \DIFdelbegin \DIFdel{valuable feature is that users can }\DIFdelend \DIFaddbegin \DIFadd{benefit that cell editors provide is enabling users to }\DIFaddend incorporate
their private information into a web UI. We created a datepicker \DIFaddbegin \DIFadd{widget
(}\DIFaddend based on the \href{https://fullcalendar.io/}{FullCalendar} plugin\DIFaddbegin \DIFadd{)}\DIFaddend ,
which can load \DIFaddbegin \DIFadd{calendar }\DIFaddend data from a Google Calendar. This makes it
convenient to enter dates into a website based on the user's personal
calendar information, without \DIFdelbegin \DIFdel{uploading that information to }\DIFdelend \DIFaddbegin \DIFadd{needing to upload a user's calendar to }\DIFaddend the
website itself.

Another benefit is that a user can choose a \DIFaddbegin \DIFadd{single }\DIFaddend preferred widget for
editing \DIFaddbegin \DIFadd{a certain type of }\DIFaddend information across different sites\DIFdelbegin \DIFdel{that typically provide a fragmented
editing experience. We }\DIFdelend \DIFaddbegin \DIFadd{. For
example, a user could use their favorite rich text editor to edit text
in various websites like blogging platforms and task trackers. To
demonstrate this capability, we }\DIFaddend built a text editor based on the
\href{https://ckeditor.com/}{CKEditor} rich text editor\DIFdelbegin \DIFdel{, and integrated
it }\DIFdelend \DIFaddbegin \DIFadd{. We used the
editor }\DIFaddend with Google's Blogger website\DIFdelbegin \DIFdel{by representing }\DIFdelend \DIFaddbegin \DIFadd{, by building a site adapter that
represented }\DIFaddend the contents of a blog post as a single table cell
containing an HTML string (shown in Figure~\ref{fig:blogger}).
\DIFdelbegin \DIFdel{If there were site adapters available for
...
\DIFdelbegin %DIFDELCMD < \hypertarget{sec:themes}{%
%DIFDELCMD < \section{Key themes}\label{sec:themes}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{sec:visions}{%
\section{Vision}\label{sec:visions}}
\DIFaddend 

\DIFdelbegin \DIFdel{Here we discuss some themes that we've explored with this work, which go
beyond our specific
approach and address broader issues in software
customization}\DIFdelend \DIFaddbegin \DIFadd{We envision data-driven customization as a broad paradigm that could
...
\DIFadd{When data is freely available outside the context of a specific
...
}\href{https://en.wikipedia.org/wiki/Yahoo!_Pipes}{\DIFadd{Yahoo Pipes}}\DIFaddend .

\DIFaddbegin \DIFadd{However, on the Web today, data is often siloed and only accessible
...
}\href{https://stratechery.com/2019/spotifys-podcast-aggregation-play/}{\DIFadd{beginning
to become exclusive}} \DIFadd{to specific platforms like Spotify. This coupling
...
\DIFadd{Some services provide APIs that mitigate these siloing effects, but APIs
...
}\href{https://www.theverge.com/2012/8/16/3248079/twitter-limits-app-developers-control}{\DIFadd{imposed
restrictions}} \DIFadd{on third-party applications that mimicked the ``mainstream
...
\DIFadd{There have been compelling suggestions for more decentralized
architectures that would give users more control of their data.
Local-first software \mbox{%DIFAUXCMD
\citep{kleppmann2019} }\hspace{0pt}%DIFAUXCMD
...
synchronization. The SOLID project \mbox{%DIFAUXCMD
\citep{berners-lee2019} }\hspace{0pt}%DIFAUXCMD
envisions a
...
\textbf{\DIFadd{Incremental:}} \DIFadd{Rather than proposing that web services be totally
rearchitected, data-driven customization suggests a more incremental
path for adding user agency to existing software.
}

\DIFadd{Data-driven customization allows for lightly augmenting a centralized
...
\DIFadd{By using third-party data extraction, data-driven customization also
...
situations where websites are }\emph{\DIFadd{neutral}}\DIFadd{: not opposed to the idea of
...
\textbf{\DIFadd{Focus on end user customization:}} \DIFadd{Having access to the data is a
...
\DIFadd{In data-driven customization, we focus heavily on this part of the
...
\DIFadd{In this sense, data-driven customization is a complementary approach to
...
as Helena \mbox{%DIFAUXCMD
\citep{chasins2018}}\hspace{0pt}%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdelend The distribution mechanism for semantic wrappers is also important for
...
Wildcard codebase. This is \DIFdelbegin \DIFdel{the simplest workable }\DIFdelend \DIFaddbegin \DIFadd{a simple }\DIFaddend solution, but \DIFdelbegin \DIFdel{has its
downsides: contributing involves a fairly
high barrier of creating a
pull request on Github, and using newly contributed wrappers }\DIFdelend \DIFaddbegin \DIFadd{makes it fairly
difficult to contribute new wrappers and }\DIFaddend requires installing a new
version of the extension \DIFaddbegin \DIFadd{to gain access to new warppers}\DIFaddend . In the future
...
scraping adapters can execute arbitrary Javascript code, which \DIFdelbegin \DIFdel{opens up the
possibility of malicious adapters being contributed.
Currently we plan
to solve this challenge with }\DIFdelend \DIFaddbegin \DIFadd{means a
malicious adapter could exfiltrate sensitive information from a page.
Approaches to security could include }\DIFaddend centralized code review, \DIFdelbegin \DIFdel{but another
approach we are considering is using or inventing a
more restricted domain-specific language for specifying scraping logic}\DIFdelend \DIFaddbegin \DIFadd{using a
...
\DIFadd{Requiring programming to create wrappers has an obvious limitation. If
...
\textbf{\DIFadd{End user wrapper creation:}} \DIFadd{If end users could create wrappers
...
scraping (such as Helena \mbox{%DIFAUXCMD
\citep{chasins2018}}\hspace{0pt}%DIFAUXCMD
...
Section~\ref{sec:architecture}), further work might be needed to enable
end-user wrapper creation}\DIFaddend .

\DIFaddbegin \textbf{\DIFadd{First party wrapper creation:}} \DIFadd{An integrated adapter installed
...
\DIFadd{With the advent of rich frontend web frameworks, structured application
state is now often available in the web client. We suspect it is
possible to create plugins for frontend frameworks that expose this
state to Wildcard with only minimal effort from the application
...
\DIFaddend \hypertarget{sec:related-work}{%
