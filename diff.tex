...
%DIF ADD paper.tex                Sat Aug 15 10:45:20 2020
...
website UI. \DIFdelbegin \DIFdel{In addition to }\DIFdelend \DIFaddbegin \DIFadd{A DOM scraping adapter performs }\DIFaddend the standard web scraping
...
individual website to make it available for customizationin Wildcard.
To make this approach viable, we have built a generic DOM scraping adapter }\DIFdelend \DIFaddbegin \DIFadd{Currently, DOM scraping adapters are programmed manually for each
website using Javascript code. Once an adapter is defined for a website,
it is added to a shared repository where it can be used by
non-programmer users for customization.
}

\DIFadd{To make the adapter definition process as easy as possible, Wildcard
provides a scraping framework }\DIFaddend which a programmer can configure with the
minimal site-specific parts. \DIFaddbegin \DIFadd{Figure~\ref{fig:scraper-code} shows an
example of the scraper code used for the Hacker News example. It
...
\DIFaddend \emph{Basic metadata}: The programmer specifies URL patterns that the
...
directly \DIFdelbegin \DIFdel{accesses }\DIFdelend \DIFaddbegin \DIFadd{access }\DIFaddend the internal state of the application, providing the
