...
%DIF ADD paper.tex                Sat Aug 15 16:36:16 2020
...
\begin{abstract}
  ...
  our experiences using Wildcard\DIFdelbegin \DIFdel{, on both its strengths
  ...
  that encourage this form of end-user customization}\DIFdelend \DIFaddbegin \DIFadd{share our broader vision for a future where end users have
...
}{[}\DIFadd{Section~\ref{sec:architecture}. In Section~\ref{sec:reflections}, we
...
entries, and inject annotations as the
tableis edited}\DIFdelend \DIFaddbegin \DIFadd{but it must also handle
...
available for customization in Wildcard. To make this
approach viable, we have built a generic DOM scraping adapter which a programmer can
configure with the minimal site-specific parts}\DIFdelend \DIFaddbegin \DIFadd{Currently, we require DOM scraping adapters to be programmed manually
...
enabling any end user to perform customizations on that website}\DIFaddend .

\DIFdelbegin \emph{\DIFdel{Basic metadata}}%DIFAUXCMD
\DIFdel{: The programmer specifies URL patterns that the
adapter should activate on , and a set of column names and types to
return in the table.
}\DIFdelend \DIFaddbegin \DIFadd{There are other ways a DOM scraping adapter could be produced which
...
adapter is a
single Javascript function that extracts relevant elements
from the page using web scraping technqiues.
For each scraped row of data, the programmer must return a unique identifier, and values for
each column in the table. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The generic adapter then wraps thisscraping function to
implement the table adapter interface}\DIFdelend \DIFaddbegin \DIFadd{To ease the burden on programmers of creating these adapters, we have
...
synchronize table edits back into the page}\DIFaddend . For example, when the user
sorts the table\DIFdelbegin \DIFdel{view, the generic adapter synchronizes the sort order to the DOM by
removing the DOM elements corresponding to }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend the \DIFdelbegin \DIFdel{rows in the table and
reinserting them back into the page in the
  new order.
}\DIFdelend \DIFaddbegin \DIFadd{DOM elements representing the table rows are moved
