...
%DIF ADD paper.tex                Sat Aug 15 17:12:18 2020
...
in \DIFdelbegin \DIFdel{the Wildcardsystem. }%DIFDELCMD < 

%DIFDELCMD < \hypertarget{abstract-interface}{%
%DIFDELCMD < \subsubsection{Abstract interface}\label{abstract-interface}}
%DIFDELCMD < 

%DIFDELCMD < %%%
\emph{\DIFdel{Returning a table}}%DIFAUXCMD
\DIFdel{: A table adapter exposes a table of data: an ordered list of records. Each record carries a unique identifier and
...
%DIFDELCMD < %%%
\DIFdel{A table adapter can update the contents of
a table at any time in
...
%DIFDELCMD < %%%
\emph{\DIFdel{Handling edits}}%DIFAUXCMD
\DIFdel{: The query engine can issue a request to a table
...
edit may result in changing the value of a form field}\DIFdelend \DIFaddbegin \DIFadd{Wildcard}\DIFaddend . \DIFaddbegin \DIFadd{These do not represent an exhaustive set of all possible
...
\DIFdelbegin \DIFdel{In addition, the query engine also sends additional information about
the combined query view to each table adapter:
}%DIFDELCMD < 

%DIFDELCMD < %%%
\emph{\DIFdel{Sorting/filtering}}%DIFAUXCMD
\DIFdel{: When the user sorts or filters the query view,
...
%DIFDELCMD < %%%
\emph{\DIFdel{Data from other tables}}%DIFAUXCMD
\DIFdel{: The query engine provides each table
...
%DIFDELCMD < %%%
\emph{\DIFdel{Currently selected record}}%DIFAUXCMD
\DIFdel{: As the user clicks around the table
...
%DIFDELCMD < %%%
\DIFdelend \hypertarget{dom-scraping-adapters}{%
