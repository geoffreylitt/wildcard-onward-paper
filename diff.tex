...
%DIF ADD paper.tex                Sat Aug 15 12:08:48 2020
...
\DIFdelbegin \DIFdel{Here we discuss some themes that we've explored with this work, which go beyond our specific approach and address broader issues in software
customization.
}\DIFdelend \DIFaddbegin \begin{itemize}
\tightlist
\item
  \DIFadd{Wildcard just one instance of a broader data-driven customization
}\DIFaddend 

  \DIFaddbegin \begin{itemize}
  \tightlist
  \item
    \DIFadd{particular view: table
  }\item
    \DIFadd{particular context: web
  }\end{itemize}
\item
  \DIFadd{here we share some of the underlying ideas. go beyond Wildcard, room
  to grow
}\end{itemize}
...
\begin{itemize}
\item
  \DIFadd{thin client architecture of web has promoted a coupling of data + app
}

  \begin{itemize}
  \tightlist
  \item
    \DIFadd{only way user can view the data is in the first-party tool
  }\item
    \DIFadd{APIs provide some flexibility in picking a client. but, APIs aren't
    meant for end user consumption. and, there are limits (eg, Twitter
    API restrictions)
  }\item
    \DIFadd{limited user agency over how they use their data
  }\item
    \DIFadd{compare to, say, opening a file locally
  }\end{itemize}
\item
  \DIFadd{Some people aiming towards ambitious rearchitectures in this
  space\ldots{}
}
  ...
  \tightlist
  \item
    \DIFadd{cite local-first
  }\item
    \DIFadd{TBL Solid
  }\end{itemize}
\item
  \DIFadd{Data-driven customization: an iterative approach towards this vision;
  complementary to more radical rearchitecting.
}\item
  \DIFadd{Start with third party extraction
}\item
  \DIFadd{Integrate across apps
}\item
  \DIFadd{open the data in a different UI
}\item
  \DIFadd{Then move to first party data providing in the client (could drive
